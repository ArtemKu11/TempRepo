...
export const actions: ActionTree<AuthState, MainState> = {
  /**
   * Reset our store
   */
  async reset ({ commit }) {
  ...
  async initAuth ({ commit, rootState, rootGetters }) {
    ...
    if (rootState.fluiddExtension.config.apiUrl === '') {
      commit('setAuthenticated', true)
      return
    }

    // Load our tokens and apply them if found.
    const keys = rootGetters['fluiddExtension/config/getTokenKeys']
