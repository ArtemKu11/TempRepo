...
export const HttpClient = (store: any) => {
    ...
    const requestInterceptor = async (config: AxiosRequestConfig) => {
        ...
        if (config.url !== '/access/refresh_jwt') {
            const isExpiring = await store.dispatch('fluiddExtension/auth/checkToken')
            if (isExpiring) {
                const token = await store.dispatch('fluiddExtension/auth/refreshTokens')
        ...
        switch (response.status) {
            case 200:
                ...
                if (
                    ...
                    store.commit('fluiddExtension/auth/setAuthenticated', true)
                }
                break
            default:
        }
    ...
    const errorInterceptor = (error: AxiosError<string | { error?: { message?: string } } | undefined>) => {
        ...
        switch (error.response.status) {
            case 500:
                ...
                console.log(error.response.status, error.message, message)
